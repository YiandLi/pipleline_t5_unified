...
class Metric():
    ...
    def update_unified_eval(self, ent_preds, _rel_preds, ent_labels, rel_labels,
        ...
        ent_s, rel_s = 0, 0
        for pred_ent_pos, pred_rel_pos, gold_ents, gold_rels, t \
                in zip(ent_pos_list, rel_pos_list, ent_labels, rel_labels, texts):
            
            pred_ent_tags = ent_preds[ent_s: ent_s + len(pred_ent_pos)]
            pred_ent_tuples = [(*i, j) for i, j in zip(pred_ent_pos, pred_ent_tags)]
            gold_ent_tuples = [(*i[0], j.replace("</s>", "").lower().replace(" ", "").split(ent_tokenizer.sep_token))
