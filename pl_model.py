...
class LitModel(pl.LightningModule):
    ...
    def generate_rel_inference_batch_by_ents(self, all_ent_vectors, ent_pred_labels_by_sentence):
        ...
        for i, ent_pred_labels in enumerate(ent_pred_labels_by_sentence):
            ...
            ent_vectors_this_sentence = ent_vectors_this_sentence[[True if i else False for i in ent_pred_labels]]
            
            # fuse ent label emb accordingly
            ture_ent_pred_labels = [i for i in ent_pred_labels if i]
            ...
            if rel_pos_:
                ...
                if self.config.agg_method.lower() == "biaffine":
                    raw_ent_vectors = self.model.rel_head_mapper(ent_vectors_this_sentence)[None,
                                      :]  # 1, ent_num, h_size
                    col_ent_vectors = self.model.rel_tail_mapper(ent_vectors_this_sentence)[None,
                                      :]  # 1, ent_num, h_size
                    rel_vector = self.model.rel_mapper(raw_ent_vectors, col_ent_vectors).squeeze(
                        0)  # ent_num, ent_num, h_size
                    rel_vector = self.model.rel_layer_norm(rel_vector)
                    ...
                    raw_ent_vectors, col_ent_vectors \
                        = ent_vectors_this_sentence.unsqueeze(1).expand(-1, len(ent_vectors_this_sentence), -1), \
                          ent_vectors_this_sentence.unsqueeze(0).expand(len(ent_vectors_this_sentence), -1, -1)
                    # aggregation
    ...
    def validation_step(self, batch_valid_data, batch_idx):  # TODO 定义 eval 过程
        batch_input_ids, batch_input_mask, ent_labels, rel_labels, texts = batch_valid_data
        ...
        with torch.no_grad():
            ...
            if span_num:
                ...
                ent_num = sum([1 for i in ent_pred_by_sent for j in i if j])  # 被预测出来的 valid ent with label 的数量
                
                if ent_num:
                    rel_inputs, rel_mask, rel_pos_list, rel_prefix_lens, agg_idx_prefix_rel, rel_vectors, document_id_lists \
                        = self.generate_rel_inference_batch_by_ents(ent_dict["ent_vector"], ent_pred_by_sent)
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        # rel_f1_score = rel_f1_score - abs(rel_recall - rel_precision) / 5
    ...
    def test_step(self, batch_test_data, batch_idx):  # TODO 定义 eval 过程
        batch_input_ids, batch_input_mask, ent_labels, rel_labels, texts = batch_test_data
        ...
        with torch.no_grad():
            ...
            span_num = sum([len(i) for i in span_idxs])
            ...
            if span_num:
                ...
                rel_inputs, rel_mask, rel_vectors = rel_inputs.to(device), rel_mask.to(device), rel_vectors.to(device)
                ...
                self.metric.update_span_eval(ent_labels, span_idxs)
                self.metric.update_unified_eval(ent_preds, rel_id_preds, ent_labels, rel_labels, self.ent_tokenizer,
                                                self.rel_tokenizer,
                                                ent_prefix_lens, rel_prefix_lens, span_idxs, rel_pos_list, texts)
            
            
            else:  # bad case, no entities is predicted
                logging.info(f"\n\n\n!! eval {batch_idx} identify none entities !!\n\n\n")
                for gold_ents, gold_rels in zip(ent_labels, rel_labels):
                    gold_ent_tuples = [
                        (*i[0], j.replace("</s>", "").lower().replace(" ", "").split(self.ent_tokenizer.sep_token))
                        for i, j in gold_ents.items()]
