...
class ScFreeModel(nn.Module):
    ...
    def __init__(self, args, encoder, ent_decoder, rel_decoder):
        ...
        self.dropout_layer = torch.nn.Dropout(encoder.config.dropout_rate)
        self.ent_layer_norm = T5LayerNorm(encoder.config.hidden_size, encoder.config.layer_norm_epsilon)
        self.rel_layer_norm = T5LayerNorm(encoder.config.hidden_size, encoder.config.layer_norm_epsilon)
        
        self.ent_head_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.ent_tail_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.ent_mapper = Biaffine(encoder.config.hidden_size // 4, encoder.config.hidden_size)
        # self.ent_mapper = torch.nn.Bilinear(encoder.config.hidden_size // 4,
        #                                     encoder.config.hidden_size // 4,
        #                                     encoder.config.hidden_size)
        
        self.rel_head_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.rel_tail_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.rel_mapper = Biaffine(encoder.config.hidden_size // 4, encoder.config.hidden_size)
        # self.rel_mapper = torch.nn.Bilinear(encoder.config.hidden_size // 4,
        #                                     encoder.config.hidden_size // 4,
        #                                     encoder.config.hidden_size)
        
        self.span_head_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.span_tail_mapper = torch.nn.Linear(encoder.config.hidden_size, encoder.config.hidden_size // 4)
        self.span_classifier = Biaffine(encoder.config.hidden_size // 4, 1)
        # self.span_classifier = torch.nn.Bilinear(encoder.config.hidden_size // 4, encoder.config.hidden_size // 4, 1)
        
        logger.info("Using Three Biaffine Layers !!")
    ...
    def get_loss(self, all_input_ids, all_input_mask,
        ...
        if self.args.train_rel and not span_loss_only:
    ...
    def get_span_logistics_loss(self, all_input_mask, last_hidden_state, ent_pos_list=None):
        ...
        raw_extend = self.span_head_mapper(last_hidden_state)
        col_extend = self.span_tail_mapper(last_hidden_state)
        span_logits = self.span_classifier(raw_extend, col_extend).squeeze()[:, None]  # batch_size, 1, seq_len, seq_len
    ...
    def get_ents_and_rels(self, last_hidden_state, ent_pos_list, rel_pos_p, rel_pos_f,
        ...
        raw_extend = self.ent_head_mapper(last_hidden_state)
        col_extend = self.ent_tail_mapper(last_hidden_state)
        raw_extend = self.dropout_layer(raw_extend)
        col_extend = self.dropout_layer(col_extend)
        span_vector = self.ent_mapper(raw_extend, col_extend)  # b_s, seq_len, hidden_state
        span_vector = self.ent_layer_norm(span_vector)
        ...
        for i, (ent_pos, ent_pos_f, _rel_pos_p, _rel_pos_f) in enumerate(
            ...
            raw_ent_vectors = self.rel_head_mapper(ent_vectors)[None, :]
            col_ent_vectors = self.rel_tail_mapper(ent_vectors)[None, :]
            raw_ent_vectors = self.dropout_layer(raw_ent_vectors)
            col_ent_vectors = self.dropout_layer(col_ent_vectors)
            
            rel_vector = self.rel_mapper(raw_ent_vectors, col_ent_vectors).squeeze()
            rel_vector = self.rel_layer_norm(rel_vector)
        ...
        return_dict["gold_rel_vector"] = torch.stack(gold_rel_vector)
    ...
    def get_ents(self, last_hidden_state, ent_pos_list):
        ...
        raw_extend = self.dropout_layer(raw_extend)
        col_extend = self.dropout_layer(col_extend)
        span_vector = self.ent_mapper(raw_extend, col_extend)
        span_vector = self.ent_layer_norm(span_vector)
